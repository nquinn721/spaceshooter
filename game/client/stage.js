	...
	Stage.prototype = {
		...
		setCanvasSize : function () {
			if(this.fullscreen){
				...
				this.debug.height = window.innerHeight;
				this.debug.style.marginLeft = 0;
			}else{
				this.canvas.width = .9 * window.innerWidth;
				this.canvas.height = .9 * window.innerHeight;
				this.canvas.style.marginLeft = '5%';
				this.debug.width = .9 * window.innerWidth;
				this.debug.height = .9 * window.innerHeight;
				this.debug.style.marginLeft = '5%';
			}
		},
